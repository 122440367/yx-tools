...
# 不限制速度和延迟，最终取速度最快的 100 个
./yx test -n 100 -sl 0 -tl 0

# 从 2000 个候选 IP 中测试，不限制速度和延迟，最终取最快的 100 个
./yx test -c 2000 -n 100 -sl 0 -tl 0
...
`-sl 0` 表示不设置下载速度下限，`-tl 0` 会按不限延迟处理。下载测速结果
默认先按速度从高到低排列，速度相同时按延迟从低到高排列；使用 `-nodl`
跳过下载测速后，结果按延迟从低到高排列。

取消筛选不等于测试全部候选 IP：`-n` 控制最终保留数量，`-c` 控制参与延迟
...
GitHub 文件会按上传顺序为每个 IP 编号，每行是：

```text
IP#序号 | 国家/地区代码 | 网络厂商 | 速度MB/s
```
